...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & 0xfffffffe; IFDEF(CONFIG_FTRACE, FTRACE_RECORD));  // TODO     // pass
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, printf("AAA, imm = %x\n", (uint32_t)imm); R(rd) = PRR(imm), PRW(imm, (PRR(imm) | src1)));
