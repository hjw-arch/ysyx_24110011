...
#define FTRACE_JAL() do { \
  if (FTRACE_IS_LINK_REG(rd)) record_ftrace(s->pc, FTRACE_CALL, s->dnpc); \
  else if (rd == 0) record_ftrace(s->pc, FTRACE_TAIL, s->dnpc); \
} while (0)

#define FTRACE_JALR() do { \
  int ftrace_rs1 = BITS(s->isa.inst.val, 19, 15); \
  if (FTRACE_IS_LINK_REG(rd)) record_ftrace(s->pc, FTRACE_CALL, s->dnpc); \
  else if (rd == 0 && FTRACE_IS_LINK_REG(ftrace_rs1) && imm == 0) record_ftrace(s->pc, FTRACE_RET, s->dnpc); \
  else if (rd == 0) record_ftrace(s->pc, FTRACE_TAIL, s->dnpc); \
} while (0)
#endif
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & 0xfffffffe; IFDEF(CONFIG_FTRACE, FTRACE_JALR()));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);      //pass
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = (int32_t)src1 < (int32_t)imm ? 1 : 0);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = src1 < imm ? 1 : 0);    // pass
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = src2 == 0 ? -1 : (src1 == 0x80000000u && src2 == -1 ? src1 : (int32_t)src1 / (int32_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = src2 == 0 ? -1 : src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = src2 == 0 ? src1 : (src1 == 0x80000000u && src2 == -1 ? 0 : (int32_t)src1 % (int32_t)src2));
