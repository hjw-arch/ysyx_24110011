...
always_ff @(posedge sck or posedge ce_n) begin
    state <= ce_n ? S_IDLE : next_state;
end

always_comb begin
    case(state) 
        S_IDLE:
            next_state = ~ce_n ? S_CMD : state;
        S_CMD:
            next_state = cnt == 3'b111 ? S_ADDR : state;
        S_ADDR:
            next_state = cnt == 3'd5 && cmd == CMD_Q_READ ? S_DUMMY : cnt == 3'd5 && cmd == CMD_Q_WRITE ? S_WRITE : state;
        S_DUMMY:
            next_state = cnt == 3'd5 ? S_READ : state;
        S_READ:
            next_state = state;
        S_WRITE:
            next_state = state;
...
always_ff @(posedge sck or posedge ce_n) begin
    if (ce_n) begin
        ...
        case(state)
            S_IDLE: cnt <= 3'b0;
            S_CMD:    cnt <= cnt == 3'b111 ? 3'b0 : cnt + 1;
...
always_ff @(posedge sck) begin
    cmd <= state == S_CMD ? {cmd[6 : 0], dio_in[0]} : cmd;
end
...
always_ff @(posedge sck) begin
    data_toggle <= cnt == 3'b101 ? 1'b1 : (state == S_READ || state == S_WRITE) ? ~data_toggle : data_toggle;
end
...
always_ff @(posedge sck) begin
    if (state == S_READ) begin
        if (data_toggle) begin
            dio_out <= read_buf[7 : 4];
        end else begin
            dio_out <= read_buf[3 : 0];
        end
    end
end
