...
//   - 当前 LS 的可提交结果，下一拍在 WB，选择 WBU 的真实写回值；
//   - 当前 WB 的相关性由寄存器堆同拍读写 bypass 吸收，不需要在这里阻塞。
...
// LS 阶段如果还不能在本拍进入 WB，例如 load 正在等待 AXI 响应，也必须阻塞。
wire rs1_block_ex = rs1_hit_ex & ~ex_can_forward;
wire rs2_block_ex = rs2_hit_ex & ~ex_can_forward;
wire rs1_block_ls = ~rs1_hit_ex & rs1_hit_ls & ~ls_can_wb;
wire rs2_block_ls = ~rs2_hit_ex & rs2_hit_ls & ~ls_can_wb;
...
assign fwd_rs1_sel = rs1_hit_ex & ex_can_forward ? FWD_SEL_LS :
					 ~rs1_hit_ex & rs1_hit_ls & ls_can_wb ? FWD_SEL_WB :
					 FWD_SEL_RF;

assign fwd_rs2_sel = rs2_hit_ex & ex_can_forward ? FWD_SEL_LS :
					 ~rs2_hit_ex & rs2_hit_ls & ls_can_wb ? FWD_SEL_WB :
					 FWD_SEL_RF;
