...
static void audio_callback(void *userdata, uint8_t *stream, int len)
{
    ...
    { // 剩余空间足够的话，直接复制，然后更新读指针的位置
        SDL_memcpy(stream, audio_pos, len_to_copy);
        audio_pos += len_to_copy;
    }
    // 如果给到SDL的字节小于len，将后面缺少的空间填0
    if (len_to_copy < len)
        SDL_memset(stream + len_to_copy, spec.silence, len - len_to_copy);

    // 更新剩余的音频字节
    audio_base[reg_count] -= len_to_copy;
}

static void audio_io_handler(uint32_t offset, int len, uint32_t is_write)
{
    if (!is_write)
        return;

    if (offset == reg_freq * 4)
        spec.freq = audio_base[reg_freq];
    if (offset == reg_channels * 4)
        spec.channels = audio_base[reg_channels];
    if (offset == reg_samples * 4)
